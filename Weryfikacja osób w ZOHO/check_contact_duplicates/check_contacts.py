...
def process_contacts(
            ...
            if duplicates:
                ...
                if best_contact and best_scoring:
                    ...
                    if source_config and "Tag" in source_config:
                        # Pobierz istniejące TAGi z kontaktu
                        existing_tags = best_contact.get("Tag", [])
                        existing_tag_names = []
                        if existing_tags:
                            if isinstance(existing_tags, list):
                                ...
                                existing_tag_names = [str(existing_tags)]
                        
                        logger.info("  🏷️ Istniejące TAGi: %s", existing_tag_names)
                        
                        # Sprawdź jaki TAG chcemy dodać
                        tag_to_add = None
                        tag_config = source_config["Tag"]
                        if tag_config["source"] == "fixed":
                            tag_to_add = tag_config["value"]
                        elif tag_config["source"] == "column":
                            column_name = tag_config["column_name"]
                            if column_name in row and pd.notna(row[column_name]):
                                tag_to_add = row[column_name]
                        
                        # Dodaj tylko jeśli TAG nie istnieje już na kontakcie
                        if tag_to_add:
                            if tag_to_add in existing_tag_names:
                                logger.info("  ⊘ TAG '%s' już istnieje na kontakcie - pomijam", tag_to_add)
                            else:
                                new_data["Tag"] = tag_to_add
                                logger.info("  🏷️ Dodaję nowy TAG do duplikatu: %s", tag_to_add)
                ...
                if source_config:
                    ...
                    logger.info("  📝 Przygotowuję dane dla nowego kontaktu")
                    logger.info("    Mapowanie kolumn: %s", mapping)
                    ...
                    for field, column in mapping.items():
                        if column and column in row and pd.notna(row[column]):
                            ...
                            logger.debug("      %s = %s (z kolumny '%s')", field, new_contact_data[field], column)
                    
                    logger.info("    Zebrane dane kontaktu: %s", new_contact_data)
                    ...
                    # Walidacja wymaganych pól (Last_Name jest wymagane w Zoho)
                    if not new_contact_data.get("Last_Name"):
                        logger.warning("  ⚠ Pominięto tworzenie - brak wymaganego pola Last_Name")
                        logger.warning("    Dane: %s", new_contact_data)
                        result_row["Status"] = "POMINIĘTY_BRAK_NAZWISKA"
                    else:
                        ...
                        logger.info("  ✅ Dodano do listy kontaktów do utworzenia")
    ...
    # Podsumowanie kontaktów pominiętych przy tworzeniu
    skipped_no_lastname = result_df[result_df["Status"] == "POMINIĘTY_BRAK_NAZWISKA"]
    if not skipped_no_lastname.empty:
        logger.warning("\n⚠ Pominięto %d kontaktów bez nazwiska (nie można utworzyć w Zoho):", len(skipped_no_lastname))
        print(f"\n⚠ Pominięto {len(skipped_no_lastname)} kontaktów bez nazwiska (wymagane przez Zoho API)")
        for _, row in skipped_no_lastname.head(10).iterrows():
            first_name = row.get("First_Name", "(brak)")
            logger.warning("  - %s (wiersz: %s)", first_name, row.name + 2)
            print(f"   - {first_name}")
        if len(skipped_no_lastname) > 10:
            print(f"   ... i {len(skipped_no_lastname) - 10} więcej")
...
def ask_for_create_confirmation(new_contacts_summary: List[Dict[str, Any]]) -> bool:
    ...
    for idx, contact in enumerate(new_contacts_summary[:10], start=1):
        data = contact['data']
        # Pokaż imię i nazwisko z danych (nie z 'name' bo to może być puste)
        display_name = f"{data.get('First_Name', '')} {data.get('Last_Name', '')}".strip() or contact['name']
        print(f"{idx}. {display_name}")
        logger.info("  %d. %s - dane: %s", idx, display_name, data)
...
def match_contacts_to_companies(
    ...
    # Słownik do grupowania: domena → lista kontaktów
    ...
    # KROK 1: Zbierz kontakty i pogrupuj według domeny
    print("\n🔍 Analizuję kontakty i grupuję według domeny emaila...")
    logger.info("KROK 1: Zbieranie i grupowanie kontaktów według domeny")
    ...
    for idx, row in contacts_with_id.iterrows():
        ...
        # Znajdź domenę firmową (z emaili lub kolumny)
        found_domain = None
        source_info = None
        
        # Jeśli mamy domenę z kolumny (a nie emaile), użyj jej bezpośrednio
        if domain_from_column and not emails:
            if not company_matcher.is_public_domain(domain_from_column):
                found_domain = domain_from_column
                source_info = f"(domena z pliku: {domain_from_column})"
                logger.info("    → Użyję domeny z pliku: %s", domain_from_column)
            else:
        ...
        if not found_domain:
            for email in emails:
                ...
                # Znaleziono niepubliczną domenę
                found_domain = domain
                source_info = email
                logger.info("    → Użyję domeny: %s (z emaila: %s)", domain, email)
                break  # Pierwsza niepubliczna domena
        
        # Jeśli nie znaleziono żadnej domeny firmowej
        if not found_domain:
            logger.info("  ⊘ Brak domeny firmowej dla %s %s (wszystkie emaile publiczne)", 
                        first_name, last_name)
            skipped_count += 1
            continue
        
        # Dodaj kontakt do grupy według domeny
        domain_groups[found_domain].append({
            "contact_id": contact_id,
        ...
        logger.debug("  → Dodano do grupy domeny: %s", found_domain)
    
    # KROK 2: Podsumowanie i przetwarzanie grup
    if not domain_groups:
        print("\n✓ Brak kontaktów do dopasowania (wszystkie mają firmę lub publiczne emaile)")
        logger.info("Brak grup do przetworzenia")
        return result_df
    
    print(f"\n📊 Pogrupowano {contacts_without_company_count} kontaktów według {len(domain_groups)} domen:")
    for domain, contacts in list(domain_groups.items())[:10]:
        print(f"   • {domain}: {len(contacts)} kontaktów")
    if len(domain_groups) > 10:
        print(f"   ... i {len(domain_groups) - 10} więcej domen")
    
    logger.info("KROK 2: Przetwarzanie %d grup domen", len(domain_groups))
    
    # Przetwarzaj każdą grupę domen
    for domain, contacts_in_group in domain_groups.items():
        logger.info("\n" + "▼" * 60)
        logger.info("GRUPA: %s (%d kontaktów)", domain, len(contacts_in_group))
        
        print(f"\n" + "=" * 80)
        print(f"🏢 DOMENA: {domain} ({len(contacts_in_group)} kontaktów)")
        print("=" * 80)
        
        # Wyszukaj firmę dla tej domeny (raz dla całej grupy)
        logger.info("  🔍 Wyszukuję firmę dla domeny: %s", domain)
        company = company_matcher.find_company_by_domain(domain)
        
        if not company:
            logger.info("  ⊘ Brak firmy dla domeny %s - pomijam grupę", domain)
            skipped_count += len(contacts_in_group)
            continue
        
        company_name = company.get("Account_Name") or company.get("Name") or "Nieznana"
        company_id = company.get("id")
        
        print(f"\n✅ Znaleziono firmę: {company_name} (ID: {company_id})")
        print(f"\nKontakty do przypisania ({len(contacts_in_group)}):")
        
        # Pokaż listę kontaktów (max 10)
        for i, contact in enumerate(contacts_in_group[:10], start=1):
            name = f"{contact['first_name']} {contact['last_name']}".strip() or "(brak nazwiska)"
            print(f"   {i}. {name} - {contact['source_info']}")
        
        if len(contacts_in_group) > 10:
            print(f"   ... i {len(contacts_in_group) - 10} więcej kontaktów")
        
        # Zapytaj o całą grupę
        print(f"\n❓ Co zrobić z tą grupą?")
        print(f"   [t]ak      - przypisz wszystkie {len(contacts_in_group)} kontaktów do {company_name}")
        print(f"   [n]ie      - pomiń wszystkie")
        print(f"   /ignoruj   - dodaj domenę '{domain}' do listy pomijanych (na stałe)")
        
        answer = input("   Wybór (t/n//ignoruj, Enter=nie): ").strip().lower()
        logger.info("Grupa %s → %s: odpowiedź '%s'", domain, company_name, answer)
        
        # Obsługa /ignoruj
        if answer == "/ignoruj":
            print(f"   🚫 Dodaję domenę '{domain}' do listy ignorowanych...")
            if company_matcher.add_ignored_domain(domain):
                print(f"   ✅ Domena '{domain}' będzie pomijana w przyszłych przebiegach")
                logger.info("Dodano '%s' do ignorowanych domen", domain)
            else:
        ...
        if answer in ["tak", "t", "yes", "y"]:
            # Przypisz wszystkie kontakty z grupy do firmy
            print(f"   ⏳ Przypisuję {len(contacts_in_group)} kontaktów do {company_name}...")
            
            for contact in contacts_in_group:
                try:
                    contact_id = contact["contact_id"]
                    update_data = {"Account_Name": str(company_id)}
                    logger.info("  Przypisuję kontakt %s (ID: %s) do firmy %s", 
                               f"{contact['first_name']} {contact['last_name']}", contact_id, company_name)
                    
                    if api_client.update_contact(contact_id, update_data):
                        # Zaktualizuj DataFrame
                        result_df.at[contact["df_index"], "Zoho_Account_Name"] = company_name
                        result_df.at[contact["df_index"], "Zoho_Account_ID"] = company_id
                        matched_count += 1
                        logger.info("    ✅ Przypisano")
                    else:
                        logger.error("    ✗ Błąd aktualizacji")
                        skipped_count += 1
                except Exception as exc:
                    logger.error("  Błąd przypisania kontaktu: %s", exc, exc_info=True)
                    skipped_count += 1
            
            print(f"   ✅ Przypisano {matched_count} kontaktów do {company_name}")
        else:
            print("   ⊘ Pominięto grupę")
            logger.info("Użytkownik odrzucił grupę %s", domain)
            skipped_count += len(contacts_in_group)
    
    print(f"\n📊 Podsumowanie dopasowania:")
    print(f"   - Domen przetworzonych: {len(domain_groups)}")
    print(f"   - Kontaktów bez firmy: {contacts_without_company_count}")
    print(f"   - Przypisano do firm: {matched_count}")
    print(f"   - Pominięto: {skipped_count}")
    
    logger.info("Zakończono dopasowanie: %d domen, %d kontaktów bez firmy, %d przypisanych, %d pominiętych", 
                len(domain_groups), contacts_without_company_count, matched_count, skipped_count)
    logger.info("🏢" * 40)
